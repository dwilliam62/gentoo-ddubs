...
local bar_font = "monospace:style=Bold:size=13"
...
oxwm.rule.add({ class = "google-chrome", tag = 2 })
...
oxwm.key.bind({ modkey }, "Y", oxwm.spawn({ "sh", "-c", "kitty -e yazi" }))
...
oxwm.key.bind({ modkey }, "Return", oxwm.spawn("kitty"))
-- Launch Dmenu
oxwm.key.bind({ modkey, "Control" }, "D", oxwm.spawn({ "sh", "-c", "dmenu_run -l 10" }))

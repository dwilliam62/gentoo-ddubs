...
oxwm.rule.add({ instance = "rofi", floating = true })
...
-- Apps
oxwm.key.bind({ modkey }, "T", oxwm.spawn("thunar"))
oxwm.key.bind({ modkey }, "Y", oxwm.spawn({ "kitty", "-e", "yazi" }))
...
-- Launcher
oxwm.key.bind({ modkey }, "D", oxwm.spawn({ "sh", "-c", "rofi-legacy.menu" }))
oxwm.key.bind({ modkey, "Shift" }, "D", oxwm.spawn({ "sh", "-c", "rofi-launcher" }))
oxwm.key.bind({ modkey, "Mod1" }, "D", oxwm.spawn("discordcanary"))
...
-- Show parser (Super+Shift+K) in terminal pager (does not grab keyboard)
oxwm.key.bind({ modkey, "Shift" }, "K", oxwm.spawn({ "sh", "-c", "oxwm-parser" }))
